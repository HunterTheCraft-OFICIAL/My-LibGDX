...
public class MainScreen extends CustomBaseScreen {
  ...
  protected void load() {
    ...
    this.image.setPosition(
        (MainClass.getInstance().getFullWidth() / 2) - (this.image.getWidth() / 2),
        (MainClass.getInstance().getFullHeight() / 2) - (this.image.getHeight() / 2)
    );

    CustomTextButton showAds = new CustomTextButton(
        "SHOW ADS", 25, MainClass.getInstance().getFullHeight() - 75, 150, 50,
        new ClickListener() {
          @Override
          public void clicked(InputEvent event, float x, float y) {
            MainClass.getInstance().showAds();
          }
        }
    );

    CustomTextButton hideAds = new CustomTextButton(
        "HIDE ADS", 25, MainClass.getInstance().getFullHeight() - 150, 150, 50,
        new ClickListener() {
          @Override
          public void clicked(InputEvent event, float x, float y) {
            MainClass.getInstance().hideAds();
          }
        }
    );

    CustomTextButton purchase = new CustomTextButton(
        "PURCHASE", MainClass.getInstance().getFullWidth() - 175, MainClass.getInstance().getFullHeight() - 75, 150, 50,
        new ClickListener() {
          @Override
          public void clicked(InputEvent event, float x, float y) {
