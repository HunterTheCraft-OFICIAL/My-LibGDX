...
public class SplashScreen extends CustomBaseScreen {
  ...
  @Override
  protected void load() {
    CustomLabel label = new CustomLabel("StressTest", Color.WHITE, CustomLabel.Sizes.LARGE);
    label.setPosition((MainClass.getInstance().getFullWidth() / 2) - (label.getWidth() / 2), (MainClass.getInstance().getFullHeight() / 2) - (label.getHeight() / 2));
